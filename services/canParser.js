...
const parseCanData = (payload, config) => {
  ...
  battery.soc = latestPayload[socId]?.soc ? (latestPayload[socId].soc / 100).toFixed(2) : 'N/A';
  battery.stackVoltage = latestPayload[socId]?.stackVoltage ?? 'N/A';
  battery.batteryStatus = latestPayload[config.canMappings.battery.batteryStatus]?.status ?? 'OFF';
  ...
  battery.current = latestPayload[config.canMappings.battery.current]?.current ?? 'N/A';
  battery.chargerCurrentDemand = latestPayload[config.canMappings.battery.chargerCurrentDemand]?.currentDemand ?? 'N/A';
  battery.chargerVoltageDemand = latestPayload[config.canMappings.battery.chargerVoltageDemand]?.voltageDemand ?? 'N/A';

  // Module Temps
  Object.keys(config.canMappings.battery.moduleTemps).forEach(module => {
    const ids = config.canMappings.battery.moduleTemps[module];
    const temps = ids.map(id => latestPayload[id]?.temps || []).flat();
  ...
  Object.keys(config.canMappings.battery.cellVoltages).forEach(module => {
    ...
    const voltages = ids.map(id => latestPayload[id]?.voltages || []).flat();
  ...
  motor.mcuEnable = latestPayload[torqueId]?.St_MCU_enable ?? 'N/A';
  motor.mcuDrivePermit = latestPayload[torqueId]?.St_MCUdriverPermit ?? 'N/A';
  motor.mcuOffPermit = latestPayload[torqueId]?.St_MCUoffPermit ?? 'N/A';
